...
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'womnipay_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'womnipay_initialize_failed', $err );
	...
	private function initialize_plugin_class( $classtovalidate ) {
		...
		\add_filter(
			'womnipay_instance_' . $classtovalidate,
			function() use ( $temp ) {
				return $temp;
			}
		);
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'Womnipay\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Womnipay\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'WomniPay is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', WOP_TEXTDOMAIN ) );
			}
